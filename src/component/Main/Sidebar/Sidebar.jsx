...
import logo from "/Auth/authLogo.png";
...
const sidebarItems = [
  ...
  // },
  {
    path: "/subscription",
    name: "Subscription ",
    icon: <CiMoneyCheck1 className="size-8" />,
  },
  // {
  //   path: "/earnings",
  //   name: "Earnings ",
...
const Sidebar = ({ isSidebarOpen, toggleSidebar }) => {
  ...
  return (
    <div>
      {/* Desktop Sidebar */}
      <div className="hidden overflow-y-auto md:block w-full md:w-[200px] lg:w-[250px] xl:w-[280px] h-full bg-[#fffbe6] fixed shadow-2xl">
        <Link to={"/"} className="flex flex-col justify-center items-center pt-5 gap-2  my-10 ">
        ...
        <ul className="flex flex-col gap-5">
          {sidebarItems.map((item) => (
            <NavLink
              ...
              to={item.path}
              className={({ isActive }) =>
                `w-[80%] mx-auto px-5 py-2 flex justify-start items-center gap-3  ${isActive ? "bg-[#fff3b8] !text-black border-l-4 border-primary" : ""
                }`
              }
            >
              {item?.icon}
        ...
        >
          <IoIosLogOut className="size-8  p-1 text-black rounded-md" />
      ...
      {/* Mobile Sidebar */}
      <div
        className={`fixed top-0 overflow-y-auto left-0 z-40 w-64 h-full bg-primary shadow-lg transform ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"
      ...
      >
        ...
        <ul className="flex flex-col gap-3 mt-10">
          {sidebarItems.map((item) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `w-[70%] mx-auto px-5 py-2 flex items-center gap-3 text-white ${isActive ? "bg-[#ceac02c4] " : ""
                }`
              }
            >
              {item?.icon}
        ...
        <button
          ...
          className="flex items-center z-[9999] gap-2 px-10 ml-5 mt-5"
        >
          <IoIosLogOut className="size-8   p-1 text-black rounded-md" />
          <span className="!text-black">Logout</span>
        </button>
